...
def test_compile_allows_schema_file_from_explicit_config(tmp_path, monkeypatch):
    ...
    args = _make_args(str(tmp_path / "out"), config="/tmp/config.toml")

    def _set_config(new_config: dict):
        config.SCHEMA_FILE = new_config["SCHEMA_FILE"]

    with (
        patch("graphqler.__main__.parse_config", return_value={"SCHEMA_FILE": "/tmp/schema.json"}),
    ...
    assert compiler_cls.call_args.kwargs["schema_file"] == "/tmp/schema.json"
